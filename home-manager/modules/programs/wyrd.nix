...
let
  cfg = config.config.programs.wyrd;
in {
  options       .programs.wyrd.enable = lib.mkEnableOption "wyrd";
  options.config.programs.wyrd.enable = lib.mkEnableOption "wyrd settings" // {
    default = config.programs.wyrd.enable;
  };

  config.home = {
    packages = lib.optionals config.programs.wyrd.enable
      (with pkgs; [ wyrd remind ]);

    file.".wyrdrc".text = lib.mkIf cfg.enable ''
