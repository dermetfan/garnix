...
let
  cfg = config.config.programs.geany;
in {
  options       .programs.geany.enable = lib.mkEnableOption "Geany";
  options.config.programs.geany.enable = with lib; mkOption {
    type = types.bool;
    default = config.programs.geany.enable;
    defaultText = "<option>config.programs.geany.enable</option>";
    description = "Whether to configure Geany.";
  };
