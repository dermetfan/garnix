...
let
  cfg = config.config.services.xscreensaver;
in {
  options.config.services.xscreensaver.enable = with lib; mkOption {
    type = types.bool;
    default = config.services.xscreensaver.enable;
    defaultText = "<option>services.xscreensaver.enable</option>";
  };
  ...
  config.home.file = lib.mkIf cfg.enable {
    ".xscreensaver".text = ''
      ...
      imageDirectory: /data/dermetfan/images/wallpapers
